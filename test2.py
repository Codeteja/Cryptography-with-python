...
def hill_dec(ciphertext , key):
    ...
    key2d[0][1] = (key2d[0][1]*(-1)) % 26
    key2d[1][0] = (key2d[0][1]*(-1)) % 26

    # multiplying by inverse of determinant
    for i in range(2):
        for j in range(2):
            key2d[i][j] = (mul_inv * key2d[i][j]) % 26
    
    # ciphertext to plaintext
    decryption = ""
    itr_count = int((ciphertext)/2)
    if (len_chek == 0):
        for i in range(itr_count):
...
plaintext = input("Enter the data to be encrypted using HILL 2*2 cipher = ")
plaintext = plaintext.upper().replace(" ","")
key = input("Enter the key of lenght exactly equal to 4 = ")
key = key.upper().replace(" ","")
ciphertext = hill_enc(plaintext , key)
print("Ciphertext = ",ciphertext)
plain = hill_dec(ciphertext , key)
